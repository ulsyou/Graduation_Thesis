...
class Activities_do extends StatefulWidget {
  final Map<String, dynamic> seasonData;

  Activities_do({required this.seasonData, Key? key}) : super(key: key);

  @override
  _Activities_doState createState() => _Activities_doState();
}

class _Activities_doState extends State<Activities_do> {
  DateTime? selectedstartDate;
  DateTime? selectedendDate;
  List<String> activitiesNames = [];
  ...
  Future<List<String>> fetchActivityFromMongoDB() async {
    final mongo.Db mongoClient =
        await mongo.Db.create('mongodb://10.0.2.2:27017/Graduation_Thesis');
    await mongoClient.open();

    final collection = mongoClient.collection('activities');
    final List<Map<String, dynamic>> documents =
        await collection.find().toList();

    final List<String> activitiesNames = documents
        .map((doc) => doc['activitiesName'] as String)
        .toSet()
        .toList();
  ...
  void initState() {
    super.initState();
    fetchActivityFromMongoDB().then((activities) {
      setState(() {
        activitiesNames = activities;
        selectedActivity = activitiesNames.isNotEmpty ? activities[0] : '';
      });
    });
  }

  Future<void> addActivitySeason() async {
    final String apiUrl = 'http://10.0.2.2:5000/do-activities/Activity';
    ...
    if (cropSeasonCode != null) {
      var response = await http.post(
        ...
        body: jsonEncode(<String, dynamic>{
          ...
          'activitiesName': selectedActivity,
          'startDate': selectedstartDate!.toIso8601String(),
          'endDate': selectedendDate!.toIso8601String(),
        }),
      );
      if (response.statusCode == 201) {
        print('activities use add successful');
      } else {
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: FutureBuilder<List<String>>(
                    future: fetchActivityFromMongoDB(),
                    builder: (context, snapshot) {
                      ...
                      return DropdownButton<String>(
                        value: selectedActivity.isNotEmpty
                            ? selectedActivity
                            : null,
                    ...
                    child: InkWell(
                      onTap: () {
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (context) =>
                                SeasonActivities(seasonData: widget.seasonData),
                          ),
                        );
                      },
